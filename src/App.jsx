...
function About({ onBook }) {
  return <section id="about" className="section-anchor bg-[#fffdfb] py-20 sm:py-24">
    <div className="mx-auto grid max-w-[1420px] items-center gap-10 px-5 sm:px-6 lg:grid-cols-[.9fr_1.1fr] lg:gap-16 lg:px-8">
      <motion.div {...A.slideLeft(0.1)} className="relative"><div className="mx-auto max-w-[440px] overflow-hidden rounded-[30px] border border-[#eadfd5] bg-[#f3e5da] shadow-[0_22px_60px_rgba(69,35,27,.12)]"><img src="assets/images/gallery-3.webp" alt="RS Bridal makeup artist introduction placeholder portrait" className="h-[520px] w-full object-cover object-center" /></div><div className="absolute -bottom-6 -right-2 max-w-[240px] rounded-[24px] border border-[#e4d4c8] bg-white p-5 shadow-xl sm:right-6"><p className="brand-serif text-[22px] text-[#6e151c]">Client details pending</p><p className="mt-2 text-[12px] leading-5 text-[#6d615b]">Replace this image and training information with the artist's real profile before launch.</p></div></motion.div>
...
class Services extends React.Component {
  ...
  render() {
    ...
    return <section id="services" className="section-anchor bg-[#fbf7f2] py-20 sm:py-24"><div className="mx-auto max-w-[1420px] px-5 sm:px-6 lg:px-8 xl:px-10">
      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{services.map((service, index) => <motion.article key={service.name} {...A.fadeUp(0.15 + index * 0.08)} className="card-lift overflow-hidden rounded-[24px] border border-[#eadfd5] bg-white"><div className="relative h-[230px] overflow-hidden bg-[#eee2d8]"><img src={service.image} alt={`${service.name} service at RS Bridal`} loading="lazy" className="service-image h-full w-full object-cover" onError={e => { if (service.fallback) { e.currentTarget.onerror = null; e.currentTarget.src = service.fallback; } }} /><span className="absolute left-4 top-4 rounded-full bg-white/92 px-3 py-1.5 text-[11px] font-bold text-[#72131a] shadow-sm">{String(index + 1).padStart(2, "0")}</span></div><div className="p-5"><h3 className="brand-serif text-[20px] text-[#241a17]">{service.name}</h3><p className="mt-2 min-h-[66px] text-[13px] leading-[22px] text-[#6a5d57]">{service.summary}</p><div className="mt-4 flex items-center justify-between gap-4 border-t border-[#eee3da] pt-3"><span className="text-[11px] font-semibold uppercase tracking-[.1em] text-[#8b6f61]">Price on consultation</span><button onClick={() => this.setState({ selected: service })} className="inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-[13px] font-bold text-[#7a111a] transition hover:bg-[#fbf0ea]">Explore <Icon name="arrow" size={15} /></button></div></div></motion.article>)}</div>
...
class ServiceModal extends React.Component {
  ...
  render() {
    ...
    return <div className="modal-backdrop fixed inset-0 z-[70] grid place-items-center bg-black/55 p-4" onMouseDown={e => e.target === e.currentTarget && onClose()}><div className="modal-panel max-h-[92vh] w-full max-w-[900px] overflow-auto rounded-[30px] bg-[#fffdfb] shadow-2xl"><div className="grid md:grid-cols-[.85fr_1.15fr]"><div className="min-h-[280px] bg-[#ead9cb]"><img src={service.image} alt={`${service.name} detailed service view`} className="h-full min-h-[280px] w-full object-cover" onError={e => { if (service.fallback) { e.currentTarget.onerror = null; e.currentTarget.src = service.fallback; } }} /></div><div className="p-6 sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="section-kicker">SERVICE DETAIL</p><h2 className="brand-serif mt-2 text-[36px] text-[#241a17]">{service.name}</h2></div><button onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-[#e1d3c9] text-[#6d171d]" aria-label="Close service details"><Icon name="close" /></button></div><p className="mt-5 text-[14px] leading-7 text-[#665a54]">{service.details}</p><div className="mt-6 rounded-[22px] border border-[#eadfd5] bg-[#fbf5ef] p-5"><strong className="text-[12px] uppercase tracking-[.12em] text-[#7b252b]">Best for</strong><p className="mt-2 text-[14px] text-[#413733]">{service.bestFor}</p></div><h3 className="mt-6 text-[12px] font-bold uppercase tracking-[.14em] text-[#7b252b]">What this can include</h3><div className="mt-3 grid gap-2">{service.includes.map(item => <div key={item} className="flex gap-3 text-[13px] leading-6 text-[#554a45]"><span className="mt-1 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#7a111a] text-white"><Icon name="check" size={12} /></span>{item}</div>)}</div><p className="mt-6 rounded-[22px] bg-[#f4e7dc] p-4 text-[12px] leading-5 text-[#6f5b52]">{service.note}</p><div className="mt-6 flex flex-wrap gap-3"><button onClick={onBook} className="btn-primary"><Icon name="calendar" size={16} /> Book This Service</button><button onClick={onClose} className="btn-secondary">Close</button></div></div></div></div></div>;
  }
}
...
class Gallery extends React.Component {
  constructor(props) { super(props); this.state = { selected: null }; }
  render() { const { onBook } = this.props; return <section id="gallery" className="section-anchor bg-[#fffdfb] py-20 sm:py-24"><div className="mx-auto max-w-[1420px] px-5 sm:px-6 lg:px-8 xl:px-10"><motion.div {...A.fadeUp(0)} className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="section-kicker">BRIDAL GALLERY</p><h2 className="section-title">Looks that inspire your wedding-day mood</h2></div><button onClick={onBook} className="btn-secondary self-start">Book Your Look</button></motion.div><div className="mt-10 grid grid-cols-2 gap-4 md:grid-cols-3">{galleryImages.map((item, index) => <motion.button key={item.src} {...A.fadeUp(0.1 + index * 0.07)} onClick={() => this.setState({ selected: item })} className={`group overflow-hidden rounded-[24px] bg-[#efe1d7] ${index === 0 ? "md:row-span-2" : ""}`}><img src={item.src} alt={item.alt} loading="lazy" className={`h-full w-full object-cover transition duration-500 group-hover:scale-[1.035] ${index === 0 ? "aspect-[3/4] min-h-[430px] md:aspect-auto" : "aspect-[4/3]"}`} /></motion.button>)}</div></div>{this.state.selected && <div className="modal-backdrop fixed inset-0 z-[80] grid place-items-center bg-black/80 p-4" onClick={() => this.setState({ selected: null })}><button className="absolute right-5 top-5 grid h-11 w-11 place-items-center rounded-full bg-white text-[#70151c]" aria-label="Close image"><Icon name="close" /></button><img src={this.state.selected.src} alt={this.state.selected.alt} className="lightbox-image max-w-[92vw] rounded-[24px]" /></div>}</section>; }
}
